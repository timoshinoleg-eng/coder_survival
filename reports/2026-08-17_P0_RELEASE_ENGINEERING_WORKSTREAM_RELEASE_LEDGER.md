...
| PR #32 — immutable-tag remediation | `bdf935195860bfeadb85bd8b7eb3984fd68358f0` | [GitHub PR #32][2]; [GO/NO-GO][3] | `releasePathImmutableTag.test.js`: targeted 2 suites / 15 tests PASS; full backend 47 suites / 447 tests total with sandbox DB skips; frontend smoke, 14 node tests and build PASS | Published by fast-forward-only update; two CI checks are in progress | Publish the local current-main resolution without force-push, then wait for CI on that final head. |
| PR #32 — current-main resolution | Local merge of published remediation with `main` `983a52461b8c01ce6d686fa6bed6a05703686361` | [Source manifest][4] | `docs/SYNC_LOG.md` retains both Sol and Manus checkpoints in chronological order | Local only; resolves current GitHub dirty state | Publish fast-forward only and record the new GitHub head. |
| Release-path runtime contract | One `git-<40-hex-reviewed-sha>` from workflow to preflight, build/migrate/restart, core smoke and offer smoke | [Manual workflow][5]; [preflight][6]; [release][7]; [core smoke][8] | Static regression rejects `latest`, short and malformed tags; compose retains required interpolation | Published; CI/independent review pending | CI/Windows runner validates PowerShell and synthetic Compose config. |
...
First, publish the current-main resolution without force-push and record its GitHub-reachable head in this ledger. Second, wait for all CI required by that final head and request a new independent Codex review. Third, only after PR #31 has actually merged, update PR #32 from fresh `main` without force-push, rerun full CI and request another independent review. No actor should merge PR #32 or perform production operations from this ledger.
