  ...
  try {
    ...
    try {
      ...
      if (myTeam?.team?.id) {
        ...
        const idx = rankResult.rows.findIndex(
          (r) => r.team_id === myTeam.team.id,
        );
        ...
        myTeam: myTeam
          ? {
              ...
              progressPercent: Math.min(
                100,
                Math.round((teamCommits / season.target_commits) * 100),
              ),
            }
          : null,
  ...
  try {
    ...
    try {
      ...
      const claimInsertResult = await client.query(
        `INSERT INTO team_battle_reward_claims (season_id, user_id, team_id, reward_payload)
        ...
        [
          ...
          userId,
          myTeam.team.id,
          JSON.stringify(season.reward_payload),
        ],
      );
      if (claimInsertResult.rows.length === 0) {
        await client.query("ROLLBACK");
