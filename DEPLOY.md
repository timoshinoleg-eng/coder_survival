...
**Target topology (2026-09-10): Cloud.ru Evolution for the backend. Vultr is retired.**
...
- CI/CD: `.github/workflows/deploy-backend.yml` (`workflow_dispatch` only, `main` only).
...
4. Run `deploy/cloudru/bootstrap-vm.sh` on the VM; Docker Compose >=2.30 is required.
...
8. Set repository/environment variable `BACKEND_HEALTH_URL=https://coder-survival-api.duckdns.org/health`.
9. Verify `WEBAPP_URL` and `FRONTEND_URL` use the same stable Vercel project origin, not a deployment-specific preview URL.
10. Run **Deploy Backend to Cloud.ru** from `main` and type `deploy`.

## Release safety

The release path is fail-closed:

- production dispatch is rejected unless it targets `main`;
- backend tests and migration idempotency must pass first;
- Docker image is built in CI and tagged by commit SHA;
- production env is validated before any DB mutation;
- secrets are passed through a raw Docker env file so Compose cannot interpolate `$`/`${...}` content;
- SSH host identity is pinned;
- API DNS must resolve to the same VM used for SSH;
- VM Docker/Compose/nginx prerequisites are checked;
- production DB reachability is checked from the VM;
