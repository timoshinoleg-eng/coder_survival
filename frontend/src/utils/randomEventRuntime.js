...
export function applyRandomEventChoice(type, action, currentState = {}, gameState = {}) {
  ...
  } else if (type === 'scope_creep' && action === 'solve') {
    handled = true;
    nextDeltas = { energyDelta: 0, depressionDelta: 3, commitsDelta: 8 };
  } else if (type === 'scope_creep' && action === 'ignore') {
    handled = true;
    nextDeltas = { energyDelta: 0, depressionDelta: -1, commitsDelta: -2 };
  } else if (type === 'merge_conflict' && action === 'solve') {
    handled = true;
    nextDeltas = { energyDelta: 0, depressionDelta: 3, commitsDelta: 5 };
  } else if (type === 'merge_conflict' && action === 'ignore') {
    handled = true;
    nextDeltas = { energyDelta: 0, depressionDelta: 5, commitsDelta: -12 };
  } else if (type === 'ci_pipeline_red' && action === 'solve') {
    handled = true;
    nextDeltas = { energyDelta: 0, depressionDelta: 1, commitsDelta: -1 };
  } else if (type === 'ci_pipeline_red' && action === 'ignore') {
    handled = true;
    nextDeltas = { energyDelta: 0, depressionDelta: 5, commitsDelta: -6 };
  } else if (type === 'slack_thread_storm' && action === 'solve') {
    handled = true;
    nextDeltas = { energyDelta: 0, depressionDelta: 1, commitsDelta: 4 };
  ...
  } else if (type === 'green_build' && action === 'solve') {
    handled = true;
    nextDeltas = { energyDelta: 0, depressionDelta: -3, commitsDelta: 15 };
  } else if (type === 'green_build' && action === 'ignore') {
    handled = true;
    nextDeltas = { energyDelta: 0, depressionDelta: 0, commitsDelta: 4 };
