...
export const MEME_TEMPLATES = {
  depression_coffee: (ctx, w, h, data = {}) => {
    ...
    const depression = Math.max(0, Math.min(200, Number(data.depression || 0)));
    const barW = w * 0.78;
    const barX = (w - barW) / 2;
    const barY = h * 0.36;
    ctx.fillStyle = '#334155';
    ctx.fillRect(barX, barY, barW, 52);
    ctx.fillStyle = depression >= 150 ? '#dc2626' : '#facc15';
    ctx.fillRect(barX, barY, barW * depression / 200, 52);
    text(ctx, data.topText || 'Моя депрессия:', w / 2, h * 0.55, 58);
    text(ctx, `${depression}/200`, w / 2, h * 0.65, 82, '#facc15', 900);
