...
export default function EventPanel({ open, onClose }) {
  ...
  const { event, showToast } = useGameState();
  ...
  if (!event) {
    return h('div', {
      ...
      h('div', { style: { padding: '14px 0', color: '#9eb6d2', fontSize: '13px' } }, 'Сейчас нет активных ивентов.')
    ]));
  }
  ...
  return h('div', {
    ...
    h('div', { style: { padding: '14px', display: 'flex', flexDirection: 'column', gap: '12px' } }, [
      h('div', null, [
        h('div', { style: { fontSize: '16px', fontWeight: 'bold', color: '#facc15', marginBottom: '4px' } }, event.title),
        h('div', { style: { fontSize: '12px', color: '#9eb6d2' } }, event.description || 'Набирай коммиты и получи награду!')
      ]),
      ...
      h('div', {
        ...
        h('div', { style: { display: 'flex', justifyContent: 'space-between', fontSize: '12px', marginBottom: '6px' } }, [
          ...
          h('span', { style: { color: '#c7ddf5', fontWeight: 'bold' } }, `${event.myContribution?.commitsContributed || 0} / ${event.targetCommits}`)
        ]),
        h('div', {
          style: { flex: 1, height: '10px', background: '#0f3460', borderRadius: '5px', overflow: 'hidden' }
        }, h('div', {
          style: {
        ...
        h('div', { style: { textAlign: 'right', fontSize: '11px', color: '#8ba1bb', marginTop: '4px' } }, `${Math.round(progress)}%`)
      ]),
      ...
      h('div', {
        ...
        h('div', { style: { fontSize: '12px', color: '#8ba1bb', marginBottom: '4px' } }, 'Награда за завершение'),
        h('div', { style: { fontSize: '13px', color: '#c7ddf5' } }, formatRewardPayload(event.rewardPayload))
      ]),
      ...
      canClaim
        ...
        : event.myContribution?.claimed
          ? h('div', { style: { textAlign: 'center', color: '#4ade80', fontSize: '13px', fontWeight: 'bold' } }, '✅ Награда получена')
          : h('div', { style: { textAlign: 'center', color: '#8ba1bb', fontSize: '12px' } }, 'Продолжай кодить, чтобы достичь цели!')
    ])
  ]));
}
